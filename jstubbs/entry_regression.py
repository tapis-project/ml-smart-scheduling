...
def get_simple_config():
    ...
    config = [
        {   
            "filename_to_write": "reg_normal.csv",
            "queue":"normal",
            ...
            "models": {
                train_knn: {
                    "param_grid": {
                        "knn__n_neighbors": np.arange(start=1, stop=6),
                    }
                },
                # train_svm: {
                #     "param_grid": {
                ...
                # train_hgbc_lr:{
                #     "param_grid": {
                #         "hgbc_lr__learning_rate": [0.01, 0.1, 1],
                #         "hgbc_lr__max_iter": [10, 100, 400, 500, 600],
                #         "hgbc_lr__max_depth": [3, 5, 9]
                # },
...
def split_train_test(df,
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, 
                                                        y, 
                                                        test_size=test_size, 
                                                        shuffle = True, ## if we make it to false, shuffle not allowed
...
def train_hgbc_lr(X_train, y_train, param_grid=None):
    """
    Train a Histogram-based Gradient Boosting Regression Tree.
    """
    p = Pipeline([
        ('scale', StandardScaler()),
        ('hgbc_lr', HistGradientBoostingRegressor()),
    ])
...
def create_queue_min_pred_bins(y_val, 
                          bin_threshold=10,
                          bin_size_factor=6,
                          nbr_bins=5):
    """
    ...
    # actual size of a bin, in minutes
    bin_size = bin_threshold * bin_size_factor
    ### print the range of minutes for each bin
    ...
    # first create a df with the y value
    s = pd.Series(y_val,
              name="y_val")
    df = s.to_frame()                         
    # first, create the column as a float
    df['queue_minutes_pred_bin'] = df['y_val'] / bin_size
    # then, cast to int 
    df = df.astype({'queue_minutes_pred_bin': 'int'})
    # for jobs in a bin number larger than the number of bins -1 (bins are 0-indexed), just 
    # put them in the largest bin. 
    df['queue_minutes_pred_bin'] = np.where(df['queue_minutes_pred_bin'] > (nbr_bins - 1), (nbr_bins - 1), df['queue_minutes_pred_bin'])
    # print the final bin counts
    #print(f"First job: {df['submit'].min()}; last job: {df['submit'].max()}")
    print("Job counts by bin:", df['queue_minutes_pred_bin'].value_counts())
    return df     

def compute_performance(model, X_test, y_test,bin_threshold, bin_size_factor, nbr_bins):
    """
    Compute the performance of a model on the test data. 
    """
    y_pred_lr = model.predict(X_test)
    y_pred_lr = np.where(y_pred_lr<0,0,y_pred_lr)
    y_test_lr = y_test
    y_pred_df = create_queue_min_pred_bins(y_pred_lr, bin_threshold, bin_size_factor, nbr_bins)
    y_test_df = create_queue_min_pred_bins(y_test_lr, bin_threshold, bin_size_factor, nbr_bins)
    y_pred = y_pred_df['queue_minutes_pred_bin']
    y_test = y_test_df['queue_minutes_pred_bin']
    r2score = r2_score(y_test_lr, y_pred_lr)
    mae = mean_absolute_error(y_test_lr, y_pred_lr)
    rmse = mean_squared_error(y_test_lr, y_pred_lr,squared=False)
    
    #print("Regression: r2 score on entire set: ", r2score))
    #print("Regression: Mean absolute error on entire set: ", mae)
    #print("Regression: mean squared error on entire set: ", rmse)
    
    acc_score_classification_after_reg =  accuracy_score(y_test, y_pred)
    print("Accuracy on entire set after classification of regression output: ", acc_score_classification_after_reg)
    ...
    print(matrix)
    return report, matrix, accuracy_scores, r2score, mae, rmse, acc_score_classification_after_reg
...
# def add_to_df(queue,window_size,model_name,win_num,win_start,
#               win_stop,nbr_jobs_window,test_r2_scores,test_maes,test_rmse,future_r2_score,future_maes, future_rmse):
#     result = ({"Queue":queue,
#                "Window size":window_size,
#                "Model name": model_name,
#                "Window number": win_num,
#                "Window start time": win_start,
#                "Window end time": win_stop,
#                "Total jobs in a window":nbr_jobs_window,
#                "Test_r2_scores":test_r2_scores,
#                "Test_maes":test_maes,
#                "Test_rmse":test_rmse,
#                "Future_r2score": future_r2_score,
#                "Future_maes":future_maes, 
...
def write_results(filename, new_result):
                with open(filename, 'a') as f:
                    f.write(f"{new_result['Queue']},{new_result['cutoff_fraction']},{new_result['Window size']},{new_result['Model name']},{new_result['Window number']},{new_result['Window start time']},{new_result['Window end time']},{new_result['Total jobs in window']},  {new_result['test_r2score']},{new_result['test_mae']},{new_result['test_rmse']},{new_result['future_r2score']},      {new_result['future_mae']},{new_result['future_rmse']}, {new_result['test_acc_score_class_after_reg']},{new_result['future_acc_score_class_after_reg']}\n")
                    f.flush()
                    #os.fsync(f.fileno())
def write(filename):
    if not os.path.exists(filename):
        print("No File")
        header = ["Queue","cutoff_fraction", "Window size", "Model name","Window number","Window start time", "Window end time", "Total jobs in window", "test_r2score", "test_mae","test_rmse", "future_r2score","future_mae","future_rmse", "test_acc_score_class_after_reg","future_acc_score_class_after_reg"  ]
        with open(filename, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(header)
    #with open(filename, 'a') as f:
        #f.write(f"Queue \t cutoff_fraction \t Window size \t Model name \t Window number \t  Window start time \t Window end time \t Total jobs in window \t test_r2score \t test_mae \t future_r2score \t future_mae \t  future_mae \t future_rmse \n")
        #f.flush()
...
def main(): 
    ...
    for idx, job in enumerate(config):
        ...
        for window_nbr, df in enumerate(dfs):
            ...
            bin_size_factor=job["bins"].get("bin_size_factor"),
            nbr_bins=job["bins"].get("nbr_bins")
            #print("bin_threshold: ", str(bin_threshold[0]))
            ...
            for model_type in job["models"].keys():
                ...
                report, matrix, scores, test_r2score, test_mae, test_rmse, test_acc_score_class_after_reg = compute_performance(model, X_test, y_test, bin_threshold[0], bin_size_factor[0], nbr_bins)
                if future is not None: 
                    print("--- Performance on FUTURE ---")
                    X_future, y_future = get_X_y(future)
                    report, matrix, scores, future_r2score, future_mae, future_rmse, future_acc_score_class_after_reg = compute_performance(model, X_future, y_future, bin_threshold[0], bin_size_factor[0], nbr_bins)
                    new_result={"Queue":queue,
                                ...
                                "test_mae":test_mae,
                                "test_rmse":test_rmse,
                                "future_r2score": future_r2score,
                                "future_mae":future_mae, 
                                "future_rmse":future_rmse,
                                "test_acc_score_class_after_reg": test_acc_score_class_after_reg,
                                "future_acc_score_class_after_reg": future_acc_score_class_after_reg}
...
if __name__ == "__main__":
    ...
    print("Printing the report of the experiments --------")
