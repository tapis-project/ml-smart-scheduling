...
def compute_performance(model, X_test, y_test,bin_threshold, bin_size_factor, nbr_bins):
    ...
    print("Regression: r2 score on entire set: ", r2_score(y_test_lr, y_pred_lr))
    print("Regression: Mean absolute error on entire set: ", mean_absolute_error(y_test_lr, y_pred_lr))
    print("Regression: root mean squared error on entire set: ", root_mean_squared_error(y_test_lr, y_pred_lr))
    print("Accuracy on entire set after classification: ", accuracy_score(y_test, y_pred))
    ...
    for idx, job in enumerate(config):
        ...
        for window_nbr, df in enumerate(dfs):
            ...
            for model_type in job["models"].keys():
                ...
                report, matrix, scores = compute_performance(model, X_test, y_test, bin_threshold[0], bin_size_factor[0], nbr_bins)
                if future is not None: 
                    print("Performance on FUTURE")
                    X_future, y_future = get_X_y(future)
                    report, matrix, scores = compute_performance(model, X_future, y_future, bin_threshold[0], bin_size_factor[0], nbr_bins)
